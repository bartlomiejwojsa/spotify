...
final class APICaller {
    ...
    public func getCategories(completion: @escaping (Result<[BrowseCategory], Error>) -> Void) {
        createRequest(
...
//                    let json = try JSONSerialization.jsonObject(with: data, options: .allowFragments)
                    let data = try JSONDecoder().decode(BrowseCategoriesResponse.self, from: data)
                    print(data)
        ...
        createRequest(
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _ , error in
                ...
                do {
                    let json = try JSONSerialization.jsonObject(with: data, options: .allowFragments)
                    print(json)
                    let data = try JSONDecoder().decode(CategoryPlaylistsResponse.self, from: data)
    ...
    public func search(with query: String, completion: @escaping (Result<[SearchResult],Error>) -> Void ) {
        createRequest(
            with: URL(string: Constants.baseAPIURL + "/search?limit=10&q=\(query.addingPercentEncoding(withAllowedCharacters: .urlUserAllowed) ?? "")&type=album,track,artist,playlist"
            ),
            type: .GET
        ) { request in
            let task = URLSession.shared.dataTask(with: request) { data, _ , error in
                ...
                do {
                    let data = try JSONDecoder().decode(SearchResponse.self, from: data)
                    var searchResults: [SearchResult] = []
                    searchResults.append(contentsOf: data.tracks.items.compactMap( { SearchResult.track(model: $0) }))
                    searchResults.append(contentsOf: data.albums.items.compactMap( { SearchResult.album(model: $0) }))
                    searchResults.append(contentsOf: data.playlists.items.compactMap( { SearchResult.playlist(model: $0) }))
                    searchResults.append(contentsOf: data.artists.items.compactMap( { SearchResult.artist(model: $0) }))
